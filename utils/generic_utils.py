...
def load_dataset_at(index, fold_index=None, normalize_timeseries=False, verbose=True) -> (np.array, np.array):
    ...
    if os.path.exists(x_train_path):
        ...
        y_train = np.load(y_train_path[1:],allow_pickle='False')
        X_test = np.load(x_test_path[1:])
        y_test = np.load(y_test_path[1:],allow_pickle='False')
    else:
        raise FileNotFoundError('File %s not found!' % (TRAIN_FILES[index]))

    is_timeseries = True
    X_train = X_train[1:] #skip the first line of data
    y_train = y_train[1:] #skip the first line of data
    # extract labels Y and normalize to [0 - (MAX - 1)] range
    str_labels = np.unique(y_train)
    for idx,label in enumerate(str_labels):
        y_train[y_train==label] = idx
